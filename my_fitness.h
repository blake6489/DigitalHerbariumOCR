...
{
	string directory="/home/blake/Desktop/OCR/search/";
	//float average=0;
...
float my_fitness(UnsharpConfigs img, vector <string> & inFiles)
{
	string directory="/home/blake/Desktop/OCR/search/";
	float average=0;
	for(int f=0;f<(int)inFiles.size();++f){
		...
		string correctText=fileRead(correctTextFile);
		string OcrText=fileRead(OcrFile);
	
		EditDistance e;
		float diff=e.CalEditDistance(correctText.c_str(),OcrText.c_str(),5000);
		float minDiff=max(correctText.length(),OcrText.length())-min(correctText.length(),OcrText.length());
		float maxDiff=max(correctText.length(),OcrText.length());
		//fit-=(diff-minDiff)/maxDiff;
		fit-=(diff)/maxDiff;
		cout<<"fit:"<<fit<<" diff:"<<diff<<" max:"<<maxDiff<<" min:"<<minDiff<<" "<<img.toString()<<endl;
		average+=fit/inFiles.size();
	}
	return average;
